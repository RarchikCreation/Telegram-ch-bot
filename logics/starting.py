...
def rotate_image_with_transparency(image, angle):
    ...
    rotated = cv2.warpAffine(image, matrix, (new_w, new_h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                             borderValue=(0, 0, 0, 0))
    return rotated
...
async def handle_photo(message: Message):
    ...
    rotated_qr = rotate_image_with_transparency(qr_img, -2)
